...
def construir_areas_candidatas(gdf, contexto):
    ...
    def filtrar_candidatas(area_busqueda, filtro_bioma, es_otro_bioma=False):
        # 1. Filtrar ecosistemas por área de búsqueda
        candidatas = ecosistemas.filterBounds(area_busqueda)
        
        # 2. Filtrar por Bioma (Exacto como el script del usuario)
        if es_otro_bioma:
        ...
        # 4. Calcular Áreas Reales (Intersección y Diferencia SINAP)
        # Obtenemos la geometría del SINAP en la zona para restarla
        sinap_geom = sinap.filterBounds(area_busqueda).geometry()
        
        def procesar_geometria(f):
            # Intersección con el área de búsqueda (Rango)
        ...
        procesados = candidatas.map(procesar_geometria).filter(ee.Filter.gt('area_ha_real', 0.01))
        return procesados
    ...
    def clasificar_y_resumir(fc):
        ...
        # Según el script del usuario: GRADO_TRAN
        conservar = fc.filter(ee.Filter.eq('GRADO_TRAN', 'Natural'))
        restaurar = fc.filter(ee.Filter.eq('GRADO_TRAN', 'Transformado'))
        
        # IMPORTANTE: Ahora usamos 'area_ha_real' que acabamos de calcular
        ha_conservar = conservar.aggregate_sum('area_ha_real').getInfo() if fc.size().getInfo() > 0 else 0
        ha_restaurar = restaurar.aggregate_sum('area_ha_real').getInfo() if fc.size().getInfo() > 0 else 0
